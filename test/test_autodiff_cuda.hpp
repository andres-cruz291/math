...
template <typename T, typename Op>
__global__ void apply_op(T *out, T *out2, T *out3, T *out4, T* out5, int numElements, Op op)
{
    int i = blockDim.x * blockIdx.x + threadIdx.x;
    if (i < numElements) {
        op(i, out, out2, out3, out4, out5);
    }
}
  ...
  , T tolerance
    ...
    for(uint i = 0; i < h_output.size(); ++i){
        for(uint j = 0; j < h_output[i].size(); ++j){
            T diff = boost::math::epsilon_difference(h_output[i][j], referenceData[i][j]);
            if (diff > tolerance){
                std::cerr << "Result verification closeness failed at element " << j << " of output " << i << "! Value: "<< h_output[i][j] << " ref "<<referenceData[i][j] << " relative_error "<< diff << std::endl;
                valid = false;
            }
        }
    }
...
{    
    ...
    if (!verifyCloseOutputs(h_out, referenceData, tolerance))
       return false;
...
{    
    ...
    watch w;
    // Execute the kernel
    apply_op<T><<<cfg.blocksPerGrid, cfg.threadsPerBlock>>>(cfg.d_outputs[0], cfg.d_outputs[1], cfg.d_outputs[2], cfg.d_outputs[3], cfg.d_outputs[4], numElements, op);
    cudaDeviceSynchronize();
    ...
    std::vector<std::vector<T>> h_out = returnOutputs(cfg);
    op(0, cfg.h_outputs[0], cfg.h_outputs[1], cfg.h_outputs[2], cfg.h_outputs[3], cfg.h_outputs[4]);
    std::vector<std::vector<T>> referenceData = defineReferenceData(cfg);
    if (!verifyCloseOutputs(h_out, referenceData, tolerance))
       return false;
    std::cout << "Test PASSED" << std::endl;
