...
template <typename RealType, size_t Order>
class fvar {
  std_array<RealType, Order + 1> v;

 public:
  using root_type = typename get_root_type<RealType>::type;  // RealType in the root fvar<RealType,Order>.

  BOOST_MATH_CUDA_ENABLED fvar() = default;

  // Initialize a variable or constant.
  BOOST_MATH_CUDA_ENABLED fvar(root_type const&, bool const is_variable);

  // RealType(cr) | RealType | RealType is copy constructible.
  BOOST_MATH_CUDA_ENABLED fvar(fvar const&) = default;

  // Be aware of implicit casting from one fvar<> type to another by this copy constructor.
  template <typename RealType2, size_t Order2>
  BOOST_MATH_CUDA_ENABLED fvar(fvar<RealType2, Order2> const&);

  // RealType(ca) | RealType | RealType is copy constructible from the arithmetic types.
  BOOST_MATH_CUDA_ENABLED explicit fvar(root_type const&);  // Initialize a constant. (No epsilon terms.)

  template <typename RealType2>
  BOOST_MATH_CUDA_ENABLED fvar(RealType2 const& ca);  // Supports any RealType2 for which static_cast<root_type>(ca) compiles.

  // r = cr | RealType& | Assignment operator.
  BOOST_MATH_CUDA_ENABLED fvar& operator=(fvar const&) = default;
  ...
  template <typename RealType2, size_t Order2>
  BOOST_MATH_CUDA_ENABLED fvar& operator+=(fvar<RealType2, Order2> const&);

  // r += ca | RealType& | Adds ar to r.
  BOOST_MATH_CUDA_ENABLED fvar& operator+=(root_type const&);
  ...
  BOOST_MATH_CUDA_ENABLED fvar operator-() const;

  // +r | RealType& | Identity Operation.
  BOOST_MATH_CUDA_ENABLED fvar const& operator+() const;

  // cr + cr2 | RealType | Binary Addition
  template <typename RealType2, size_t Order2>
  BOOST_MATH_CUDA_ENABLED promote<fvar, fvar<RealType2, Order2>> operator+(fvar<RealType2, Order2> const&) const;

  // cr + ca | RealType | Binary Addition
  BOOST_MATH_CUDA_ENABLED fvar operator+(root_type const&) const;

  // ca + cr | RealType | Binary Addition
  template <typename RealType2, size_t Order2>
  BOOST_MATH_CUDA_ENABLED friend fvar<RealType2, Order2> operator+(typename fvar<RealType2, Order2>::root_type const&,
  ...
  template <typename RealType2, size_t Order2>
  BOOST_MATH_CUDA_ENABLED bool operator==(fvar<RealType2, Order2> const&) const;

  // cr == ca | bool | Equality Comparison
  BOOST_MATH_CUDA_ENABLED bool operator==(root_type const&) const;

  // ca == cr | bool | Equality Comparison
  template <typename RealType2, size_t Order2>
  BOOST_MATH_CUDA_ENABLED friend bool operator==(typename fvar<RealType2, Order2>::root_type const&, fvar<RealType2, Order2> const&);
  ...
  BOOST_MATH_CUDA_ENABLED get_type_at<RealType, sizeof...(Orders)> at(size_t order, Orders... orders) const;

  template <typename... Orders>
  BOOST_MATH_CUDA_ENABLED get_type_at<fvar, sizeof...(Orders)> derivative(Orders... orders) const;

  BOOST_MATH_CUDA_ENABLED const RealType& operator[](size_t) const;

  BOOST_MATH_CUDA_ENABLED fvar inverse() const;  // Multiplicative inverse.

  BOOST_MATH_CUDA_ENABLED fvar& negate();  // Negate and return reference to *this.
  ...
  BOOST_MATH_CUDA_ENABLED explicit operator T() const;  // Must be explicit; multiprecision has trouble without the std::enable_if

  BOOST_MATH_CUDA_ENABLED fvar& set_root(root_type const&);

  // Apply coefficients using horner method.
  template <typename Func, typename Fvar, typename... Fvars>
  BOOST_MATH_CUDA_ENABLED promote<fvar<RealType, Order>, Fvar, Fvars...> apply_coefficients(size_t const order,
                                                                    Func const& f,
                                                                    Fvar const& cr,
                                                                    Fvars&&... fvars) const;

  template <typename Func>
  BOOST_MATH_CUDA_ENABLED fvar apply_coefficients(size_t const order, Func const& f) const;

  // Use when function returns derivative(i)/factorial(i) and may have some infinite derivatives.
  template <typename Func, typename Fvar, typename... Fvars>
  BOOST_MATH_CUDA_ENABLED promote<fvar<RealType, Order>, Fvar, Fvars...> apply_coefficients_nonhorner(size_t const order,
                                                                              Func const& f,
                                                                              Fvar const& cr,
                                                                              Fvars&&... fvars) const;

  template <typename Func>
  BOOST_MATH_CUDA_ENABLED fvar apply_coefficients_nonhorner(size_t const order, Func const& f) const;

  // Apply derivatives using horner method.
  template <typename Func, typename Fvar, typename... Fvars>
  BOOST_MATH_CUDA_ENABLED promote<fvar<RealType, Order>, Fvar, Fvars...> apply_derivatives(size_t const order,
                                                                   Func const& f,
                                                                   Fvar const& cr,
                                                                   Fvars&&... fvars) const;

  template <typename Func>
  BOOST_MATH_CUDA_ENABLED fvar apply_derivatives(size_t const order, Func const& f) const;

  // Use when function returns derivative(i) and may have some infinite derivatives.
  template <typename Func, typename Fvar, typename... Fvars>
  BOOST_MATH_CUDA_ENABLED promote<fvar<RealType, Order>, Fvar, Fvars...> apply_derivatives_nonhorner(size_t const order,
                                                                             Func const& f,
                                                                             Fvar const& cr,
                                                                             Fvars&&... fvars) const;

  template <typename Func>
  BOOST_MATH_CUDA_ENABLED fvar apply_derivatives_nonhorner(size_t const order, Func const& f) const;

 private:
  BOOST_MATH_CUDA_ENABLED RealType epsilon_inner_product(size_t z0,
                                 size_t isum0,
                                 size_t m0,
                                 fvar const& cr,
                                 size_t z1,
  ...
  BOOST_MATH_CUDA_ENABLED fvar epsilon_multiply(size_t z0, size_t isum0, fvar const& cr, size_t z1, size_t isum1) const;

  BOOST_MATH_CUDA_ENABLED fvar epsilon_multiply(size_t z0, size_t isum0, root_type const& ca) const;

  BOOST_MATH_CUDA_ENABLED fvar inverse_apply() const;

  BOOST_MATH_CUDA_ENABLED fvar& multiply_assign_by_root_type(bool is_root, root_type const&);
  ...
  BOOST_MATH_CUDA_ENABLED void fvar_cpp11(std::true_type, RootType const& ca, bool const is_variable);

  template <typename RootType>
  BOOST_MATH_CUDA_ENABLED void fvar_cpp11(std::false_type, RootType const& ca, bool const is_variable);

  template <typename... Orders>
  BOOST_MATH_CUDA_ENABLED get_type_at<RealType, sizeof...(Orders)> at_cpp11(std::true_type, size_t order, Orders... orders) const;

  template <typename... Orders>
  BOOST_MATH_CUDA_ENABLED get_type_at<RealType, sizeof...(Orders)> at_cpp11(std::false_type, size_t order, Orders... orders) const;

  template <typename SizeType>
  BOOST_MATH_CUDA_ENABLED fvar epsilon_multiply_cpp11(std::true_type,
                              SizeType z0,
                              size_t isum0,
                              fvar const& cr,
                              size_t z1,
  ...
  template <typename SizeType>
  BOOST_MATH_CUDA_ENABLED fvar epsilon_multiply_cpp11(std::false_type,
                              SizeType z0,
                              size_t isum0,
                              fvar const& cr,
                              size_t z1,
  ...
  template <typename SizeType>
  BOOST_MATH_CUDA_ENABLED fvar epsilon_multiply_cpp11(std::true_type, SizeType z0, size_t isum0, root_type const& ca) const;

  template <typename SizeType>
  BOOST_MATH_CUDA_ENABLED fvar epsilon_multiply_cpp11(std::false_type, SizeType z0, size_t isum0, root_type const& ca) const;

  template <typename RootType>
  BOOST_MATH_CUDA_ENABLED fvar& multiply_assign_by_root_type_cpp11(std::true_type, bool is_root, RootType const& ca);

  template <typename RootType>
  BOOST_MATH_CUDA_ENABLED fvar& multiply_assign_by_root_type_cpp11(std::false_type, bool is_root, RootType const& ca);

  template <typename RootType>
  BOOST_MATH_CUDA_ENABLED fvar& negate_cpp11(std::true_type, RootType const&);

  template <typename RootType>
  BOOST_MATH_CUDA_ENABLED fvar& negate_cpp11(std::false_type, RootType const&);

  template <typename RootType>
  BOOST_MATH_CUDA_ENABLED fvar& set_root_cpp11(std::true_type, RootType const& root);

  template <typename RootType>
  BOOST_MATH_CUDA_ENABLED fvar& set_root_cpp11(std::false_type, RootType const& root);
#endif
};
...
// fabs(cr1) | RealType
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> fabs(fvar<RealType, Order> const&);

// abs(cr1) | RealType
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> abs(fvar<RealType, Order> const&);

// ceil(cr1) | RealType
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> ceil(fvar<RealType, Order> const&);

// floor(cr1) | RealType
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> floor(fvar<RealType, Order> const&);

// exp(cr1) | RealType
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> exp(fvar<RealType, Order> const&);

// pow(cr, ca) | RealType
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> pow(fvar<RealType, Order> const&, typename fvar<RealType, Order>::root_type const&);

// pow(ca, cr) | RealType
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> pow(typename fvar<RealType, Order>::root_type const&, fvar<RealType, Order> const&);

// pow(cr1, cr2) | RealType
template <typename RealType1, size_t Order1, typename RealType2, size_t Order2>
BOOST_MATH_CUDA_ENABLED promote<fvar<RealType1, Order1>, fvar<RealType2, Order2>> pow(fvar<RealType1, Order1> const&,
                                                              fvar<RealType2, Order2> const&);

// sqrt(cr1) | RealType
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> sqrt(fvar<RealType, Order> const&);

// log(cr1) | RealType
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> log(fvar<RealType, Order> const&);

// frexp(cr1, &i) | RealType
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> frexp(fvar<RealType, Order> const&, int*);

// ldexp(cr1, i) | RealType
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> ldexp(fvar<RealType, Order> const&, int);

// cos(cr1) | RealType
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> cos(fvar<RealType, Order> const&);

// sin(cr1) | RealType
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> sin(fvar<RealType, Order> const&);

// asin(cr1) | RealType
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> asin(fvar<RealType, Order> const&);

// tan(cr1) | RealType
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> tan(fvar<RealType, Order> const&);

// atan(cr1) | RealType
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> atan(fvar<RealType, Order> const&);

// atan2(cr, ca) | RealType
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> atan2(fvar<RealType, Order> const&, typename fvar<RealType, Order>::root_type const&);

// atan2(ca, cr) | RealType
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> atan2(typename fvar<RealType, Order>::root_type const&, fvar<RealType, Order> const&);

// atan2(cr1, cr2) | RealType
template <typename RealType1, size_t Order1, typename RealType2, size_t Order2>
BOOST_MATH_CUDA_ENABLED promote<fvar<RealType1, Order1>, fvar<RealType2, Order2>> atan2(fvar<RealType1, Order1> const&,
                                                                fvar<RealType2, Order2> const&);

// fmod(cr1,cr2) | RealType
template <typename RealType1, size_t Order1, typename RealType2, size_t Order2>
BOOST_MATH_CUDA_ENABLED promote<fvar<RealType1, Order1>, fvar<RealType2, Order2>> fmod(fvar<RealType1, Order1> const&,
                                                               fvar<RealType2, Order2> const&);

// round(cr1) | RealType
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> round(fvar<RealType, Order> const&);

// iround(cr1) | int
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED int iround(fvar<RealType, Order> const&);

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED long lround(fvar<RealType, Order> const&);

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED long long llround(fvar<RealType, Order> const&);

// trunc(cr1) | RealType
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> trunc(fvar<RealType, Order> const&);

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED long double truncl(fvar<RealType, Order> const&);

// itrunc(cr1) | int
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED int itrunc(fvar<RealType, Order> const&);

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED long long lltrunc(fvar<RealType, Order> const&);

// Additional functions
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> acos(fvar<RealType, Order> const&);

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> acosh(fvar<RealType, Order> const&);

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> asinh(fvar<RealType, Order> const&);

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> atanh(fvar<RealType, Order> const&);

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> cosh(fvar<RealType, Order> const&);

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> digamma(fvar<RealType, Order> const&);

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> erf(fvar<RealType, Order> const&);

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> erfc(fvar<RealType, Order> const&);

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> lambert_w0(fvar<RealType, Order> const&);

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> lgamma(fvar<RealType, Order> const&);

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> sinc(fvar<RealType, Order> const&);

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> sinh(fvar<RealType, Order> const&);

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> tanh(fvar<RealType, Order> const&);

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> tgamma(fvar<RealType, Order> const&);
...
template <typename RealType, size_t Order, size_t... Orders>
BOOST_MATH_CUDA_ENABLED autodiff_fvar<RealType, Order, Orders...> make_fvar(RealType const& ca) {
...
BOOST_MATH_CUDA_ENABLED auto make_fvar_for_tuple(std::index_sequence<Is...>, RealType const& ca) {
  return make_fvar<RealType, zero<Is>::value..., Order>(ca);
}

template <typename RealType, size_t... Orders, size_t... Is, typename... RealTypes>
BOOST_MATH_CUDA_ENABLED auto make_ftuple_impl(std::index_sequence<Is...>, RealTypes const&... ca) {
...
BOOST_MATH_CUDA_ENABLED auto make_ftuple(RealTypes const&... ca) {
...
#ifndef BOOST_NO_CXX17_IF_CONSTEXPR
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order>::fvar(root_type const& ca, bool const is_variable) {
  if constexpr (is_fvar<RealType>::value) {
    v.front() = RealType(ca, is_variable);
    if constexpr (0 < Order)
      fill(v.begin() + 1, v.end(), static_cast<RealType>(0));
  } else {
    v.front() = ca;
    if constexpr (0 < Order)
      v[1] = static_cast<root_type>(static_cast<int>(is_variable));
    if constexpr (1 < Order)
      fill(v.begin() + 2, v.end(), static_cast<RealType>(0));
  }
}
#endif
...
template <typename RealType, size_t Order>
template <typename RealType2, size_t Order2>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order>::fvar(fvar<RealType2, Order2> const& cr) {
  for (size_t i = 0; i <= (min)(Order, Order2); ++i)
    v[i] = static_cast<RealType>(cr.v[i]);
  if BOOST_AUTODIFF_IF_CONSTEXPR (Order2 < Order)
    fill(v.begin() + (Order2 + 1), v.end(), static_cast<RealType>(0));
}

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order>::fvar(root_type const& ca) : v{{static_cast<RealType>(ca)}} {}

// Can cause compiler error if RealType2 cannot be cast to root_type.
template <typename RealType, size_t Order>
template <typename RealType2>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order>::fvar(RealType2 const& ca) : v{{static_cast<RealType>(ca)}} {}
...
template <typename RealType, size_t Order>
template <typename RealType2, size_t Order2>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order>& fvar<RealType, Order>::operator+=(fvar<RealType2, Order2> const& cr) {
  for (size_t i = 0; i <= (min)(Order, Order2); ++i)
    v[i] += cr.v[i];
  return *this;
}
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order>& fvar<RealType, Order>::operator+=(root_type const& ca) {
  v.front() += ca;
  return *this;
}
...
template <typename RealType2, size_t Order2>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order>& fvar<RealType, Order>::operator*=(fvar<RealType2, Order2> const& cr) {
  using diff_t = typename std_array<RealType, Order + 1>::difference_type;
  promote<RealType, RealType2> const zero(0);
  if BOOST_AUTODIFF_IF_CONSTEXPR (Order <= Order2)
    for (size_t i = 0, j = Order; i <= Order; ++i, --j)
      v[j] = inner_product(v.cbegin(), v.cend() - diff_t(i), cr.v.crbegin() + diff_t(i), zero);
  else {
    for (size_t i = 0, j = Order; i <= Order - Order2; ++i, --j)
      v[j] = inner_product(cr.v.cbegin(), cr.v.cend(), v.crbegin() + diff_t(i), zero);
    for (size_t i = Order - Order2 + 1, j = Order2 - 1; i <= Order; ++i, --j)
      v[j] = inner_product(cr.v.cbegin(), cr.v.cbegin() + diff_t(j + 1), v.crbegin() + diff_t(i), zero);
  }
  return *this;
}
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order>& fvar<RealType, Order>::operator/=(root_type const& ca) {
  for_each(v.begin(), v.end(), [&ca](RealType& x) { x /= ca; });
  return *this;
}

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> fvar<RealType, Order>::operator-() const {
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> const& fvar<RealType, Order>::operator+() const {
  return *this;
}

template <typename RealType, size_t Order>
template <typename RealType2, size_t Order2>
BOOST_MATH_CUDA_ENABLED promote<fvar<RealType, Order>, fvar<RealType2, Order2>> fvar<RealType, Order>::operator+(
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> fvar<RealType, Order>::operator+(root_type const& ca) const {
  fvar<RealType, Order> retval(*this);
  retval.v.front() += ca;
  return retval;
}
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> operator+(typename fvar<RealType, Order>::root_type const& ca,
...
BOOST_MATH_CUDA_ENABLED promote<fvar<RealType, Order>, fvar<RealType2, Order2>> fvar<RealType, Order>::operator*(
  ...
  if BOOST_AUTODIFF_IF_CONSTEXPR (Order < Order2)
    for (size_t i = 0, j = Order, k = Order2; i <= Order2; ++i, j && --j, --k)
      retval.v[i] = inner_product(v.cbegin(), v.cend() - diff_t(j), cr.v.crbegin() + diff_t(k), zero);
  else
    for (size_t i = 0, j = Order2, k = Order; i <= Order; ++i, j && --j, --k)
      retval.v[i] = inner_product(cr.v.cbegin(), cr.v.cend() - diff_t(j), v.crbegin() + diff_t(k), zero);
  return retval;
}
...
template <typename RealType, size_t Order>
template <typename RealType2, size_t Order2>
BOOST_MATH_CUDA_ENABLED bool fvar<RealType, Order>::operator==(fvar<RealType2, Order2> const& cr) const {
  return v.front() == cr.v.front();
}

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED bool fvar<RealType, Order>::operator==(root_type const& ca) const {
  return v.front() == ca;
}

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED bool operator==(typename fvar<RealType, Order>::root_type const& ca, fvar<RealType, Order> const& cr) {
...
template <typename RealType, size_t Order>
template <typename Func, typename Fvar, typename... Fvars>
BOOST_MATH_CUDA_ENABLED promote<fvar<RealType, Order>, Fvar, Fvars...> fvar<RealType, Order>::apply_coefficients(
    size_t const order,
    Func const& f,
    Fvar const& cr,
    Fvars&&... fvars) const {
  fvar<RealType, Order> const epsilon = fvar<RealType, Order>(*this).set_root(0);
  size_t i = (min)(order, order_sum);
...
template <typename RealType, size_t Order>
template <typename Func>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> fvar<RealType, Order>::apply_coefficients(size_t const order, Func const& f) const {
  fvar<RealType, Order> const epsilon = fvar<RealType, Order>(*this).set_root(0);
#ifndef BOOST_NO_CXX17_IF_CONSTEXPR
  size_t i = (min)(order, order_sum);
...
template <typename RealType, size_t Order>
template <typename Func, typename Fvar, typename... Fvars>
BOOST_MATH_CUDA_ENABLED promote<fvar<RealType, Order>, Fvar, Fvars...> fvar<RealType, Order>::apply_coefficients_nonhorner(
    size_t const order,
    Func const& f,
    Fvar const& cr,
    Fvars&&... fvars) const {
  ...
  size_t const i_max = (min)(order, order_sum);
...
template <typename RealType, size_t Order>
template <typename Func>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> fvar<RealType, Order>::apply_coefficients_nonhorner(size_t const order,
                                                                          Func const& f) const {
  fvar<RealType, Order> const epsilon = fvar<RealType, Order>(*this).set_root(0);
  fvar<RealType, Order> epsilon_i = fvar<RealType, Order>(1);  // epsilon to the power of i
...
template <typename RealType, size_t Order>
template <typename Func, typename Fvar, typename... Fvars>
BOOST_MATH_CUDA_ENABLED promote<fvar<RealType, Order>, Fvar, Fvars...> fvar<RealType, Order>::apply_derivatives(
    size_t const order,
    Func const& f,
    Fvar const& cr,
    Fvars&&... fvars) const {
...
template <typename RealType, size_t Order>
template <typename Func>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> fvar<RealType, Order>::apply_derivatives(size_t const order, Func const& f) const {
  fvar<RealType, Order> const epsilon = fvar<RealType, Order>(*this).set_root(0);
#ifndef BOOST_NO_CXX17_IF_CONSTEXPR
  size_t i = (min)(order, order_sum);
#else  // ODR-use of static constexpr
...
template <typename RealType, size_t Order>
template <typename Func, typename Fvar, typename... Fvars>
BOOST_MATH_CUDA_ENABLED promote<fvar<RealType, Order>, Fvar, Fvars...> fvar<RealType, Order>::apply_derivatives_nonhorner(
    size_t const order,
    Func const& f,
    Fvar const& cr,
    Fvars&&... fvars) const {
...
template <typename RealType, size_t Order>
template <typename Func>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> fvar<RealType, Order>::apply_derivatives_nonhorner(size_t const order,
                                                                         Func const& f) const {
  fvar<RealType, Order> const epsilon = fvar<RealType, Order>(*this).set_root(0);
  fvar<RealType, Order> epsilon_i = fvar<RealType, Order>(1);  // epsilon to the power of i
  fvar<RealType, Order> accumulator = fvar<RealType, Order>(f(0u));
...
template <typename RealType, size_t Order>
template <typename... Orders>
BOOST_MATH_CUDA_ENABLED get_type_at<RealType, sizeof...(Orders)> fvar<RealType, Order>::at(size_t order, Orders... orders) const {
...
template <typename RealType, size_t Order>
template <typename... Orders>
BOOST_MATH_CUDA_ENABLED get_type_at<fvar<RealType, Order>, sizeof...(Orders)> fvar<RealType, Order>::derivative(
    Orders... orders) const {
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED const RealType& fvar<RealType, Order>::operator[](size_t i) const {
  return v[i];
}

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED RealType fvar<RealType, Order>::epsilon_inner_product(size_t z0,
                                                      size_t const isum0,
                                                      size_t const m0,
                                                      fvar<RealType, Order> const& cr,
                                                      size_t z1,
...
#ifndef BOOST_NO_CXX17_IF_CONSTEXPR
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> fvar<RealType, Order>::epsilon_multiply(size_t z0,
                                                              size_t isum0,
                                                              fvar<RealType, Order> const& cr,
                                                              size_t z1,
                                                              size_t isum1) const {
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> fvar<RealType, Order>::inverse() const {
  return static_cast<root_type>(*this) == 0 ? inverse_apply() : 1 / *this;
}

#ifndef BOOST_NO_CXX17_IF_CONSTEXPR
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order>& fvar<RealType, Order>::negate() {
  if constexpr (is_fvar<RealType>::value)
    for_each(v.begin(), v.end(), [](RealType& r) { r.negate(); });
  else
    for_each(v.begin(), v.end(), [](RealType& a) { a = -a; });
  return *this;
}
#endif
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> fvar<RealType, Order>::inverse_apply() const {
...
#ifndef BOOST_NO_CXX17_IF_CONSTEXPR
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order>& fvar<RealType, Order>::multiply_assign_by_root_type(bool is_root,
                                                                           root_type const& ca) {
  auto itr = v.begin();
  if constexpr (is_fvar<RealType>::value) {
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order>::operator root_type() const {
  return static_cast<root_type>(v.front());
}

template <typename RealType, size_t Order>
template <typename T, typename>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order>::operator T() const {
  return static_cast<T>(static_cast<root_type>(v.front()));
}

#ifndef BOOST_NO_CXX17_IF_CONSTEXPR
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order>& fvar<RealType, Order>::set_root(root_type const& root) {
  if constexpr (is_fvar<RealType>::value)
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> fabs(fvar<RealType, Order> const& cr) {
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> abs(fvar<RealType, Order> const& cr) {
  return fabs(cr);
}

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> ceil(fvar<RealType, Order> const& cr) {
  using std::ceil;
  return fvar<RealType, Order>(ceil(static_cast<typename fvar<RealType, Order>::root_type>(cr)));
}

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> floor(fvar<RealType, Order> const& cr) {
  using std::floor;
  return fvar<RealType, Order>(floor(static_cast<typename fvar<RealType, Order>::root_type>(cr)));
}

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> exp(fvar<RealType, Order> const& cr) {
  using std::exp;
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> pow(fvar<RealType, Order> const& x,
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> pow(typename fvar<RealType, Order>::root_type const& x,
...
template <typename RealType1, size_t Order1, typename RealType2, size_t Order2>
BOOST_MATH_CUDA_ENABLED promote<fvar<RealType1, Order1>, fvar<RealType2, Order2>> pow(fvar<RealType1, Order1> const& x,
  ...
  if BOOST_AUTODIFF_IF_CONSTEXPR (order == 0)
    ...
    std_array<fvar<root_type, order>, order + 1> lognx;
    ...
    lognx[(min)(size_t(1), order)] = log(make_fvar<root_type, order>(x0));
#endif
    for (size_t i = 1; i < order; ++i)
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> sqrt(fvar<RealType, Order> const& cr) {
  using std::sqrt;
  using root_type = typename fvar<RealType, Order>::root_type;
  constexpr size_t order = fvar<RealType, Order>::order_sum;
  root_type derivatives[order + 1];
...
#else  // for compilers that compile this branch when order=0.
    derivatives[(min)(size_t(1), order)] = numerator / *derivatives;
#endif
    using diff_t = typename std_array<RealType, Order + 1>::difference_type;
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> log(fvar<RealType, Order> const& cr) {
  using std::log;
  using root_type = typename fvar<RealType, Order>::root_type;
  constexpr size_t order = fvar<RealType, Order>::order_sum;
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> cos(fvar<RealType, Order> const& cr) {
  BOOST_MATH_STD_USING
  using root_type = typename fvar<RealType, Order>::root_type;
  constexpr size_t order = fvar<RealType, Order>::order_sum;
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> sin(fvar<RealType, Order> const& cr) {
  BOOST_MATH_STD_USING
  using root_type = typename fvar<RealType, Order>::root_type;
  constexpr size_t order = fvar<RealType, Order>::order_sum;
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> asin(fvar<RealType, Order> const& cr) {
  using std::asin;
  using root_type = typename fvar<RealType, Order>::root_type;
  constexpr size_t order = fvar<RealType, Order>::order_sum;
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> tan(fvar<RealType, Order> const& cr) {
  using std::tan;
  using root_type = typename fvar<RealType, Order>::root_type;
  constexpr size_t order = fvar<RealType, Order>::order_sum;
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> atan(fvar<RealType, Order> const& cr) {
  using std::atan;
  using root_type = typename fvar<RealType, Order>::root_type;
  constexpr size_t order = fvar<RealType, Order>::order_sum;
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> atan2(fvar<RealType, Order> const& cr,
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> atan2(typename fvar<RealType, Order>::root_type const& ca,
                            fvar<RealType, Order> const& cr) {
  using std::atan2;
  using root_type = typename fvar<RealType, Order>::root_type;
  constexpr size_t order = fvar<RealType, Order>::order_sum;
...
template <typename RealType1, size_t Order1, typename RealType2, size_t Order2>
BOOST_MATH_CUDA_ENABLED promote<fvar<RealType1, Order1>, fvar<RealType2, Order2>> atan2(fvar<RealType1, Order1> const& cr1,
...
template <typename RealType1, size_t Order1, typename RealType2, size_t Order2>
BOOST_MATH_CUDA_ENABLED promote<fvar<RealType1, Order1>, fvar<RealType2, Order2>> fmod(fvar<RealType1, Order1> const& cr1,
                                                               fvar<RealType2, Order2> const& cr2) {
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> round(fvar<RealType, Order> const& cr) {
  using boost::math::round;
  return fvar<RealType, Order>(round(static_cast<typename fvar<RealType, Order>::root_type>(cr)));
}

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED int iround(fvar<RealType, Order> const& cr) {
  using boost::math::iround;
  return iround(static_cast<typename fvar<RealType, Order>::root_type>(cr));
}

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED long lround(fvar<RealType, Order> const& cr) {
  using boost::math::lround;
  return lround(static_cast<typename fvar<RealType, Order>::root_type>(cr));
}

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED long long llround(fvar<RealType, Order> const& cr) {
  using boost::math::llround;
  return llround(static_cast<typename fvar<RealType, Order>::root_type>(cr));
}

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> trunc(fvar<RealType, Order> const& cr) {
  using boost::math::trunc;
  return fvar<RealType, Order>(trunc(static_cast<typename fvar<RealType, Order>::root_type>(cr)));
}

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED long double truncl(fvar<RealType, Order> const& cr) {
  using std::truncl;
  return truncl(static_cast<typename fvar<RealType, Order>::root_type>(cr));
}

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED int itrunc(fvar<RealType, Order> const& cr) {
  using boost::math::itrunc;
  return itrunc(static_cast<typename fvar<RealType, Order>::root_type>(cr));
}

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED long long lltrunc(fvar<RealType, Order> const& cr) {
  using boost::math::lltrunc;
  return lltrunc(static_cast<typename fvar<RealType, Order>::root_type>(cr));
}

template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED std::ostream& operator<<(std::ostream& out, fvar<RealType, Order> const& cr) {
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> acos(fvar<RealType, Order> const& cr) {
  using std::acos;
  using root_type = typename fvar<RealType, Order>::root_type;
  constexpr size_t order = fvar<RealType, Order>::order_sum;
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> acosh(fvar<RealType, Order> const& cr) {
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> asinh(fvar<RealType, Order> const& cr) {
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> atanh(fvar<RealType, Order> const& cr) {
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> cosh(fvar<RealType, Order> const& cr) {
  BOOST_MATH_STD_USING
  using root_type = typename fvar<RealType, Order>::root_type;
  constexpr size_t order = fvar<RealType, Order>::order_sum;
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> digamma(fvar<RealType, Order> const& cr) {
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> erf(fvar<RealType, Order> const& cr) {
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> erfc(fvar<RealType, Order> const& cr) {
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> lambert_w0(fvar<RealType, Order> const& cr) {
  using std::exp;
  ...
  *derivatives = lambert_w0(static_cast<root_type>(cr));
  if BOOST_AUTODIFF_IF_CONSTEXPR (order == 0)
    ...
    if BOOST_AUTODIFF_IF_CONSTEXPR (order == 1)
      ...
      std_array<root_type, order> coef{{-1, -1}};  // as in derivatives[2].
      for (size_t n = 3; n <= order; ++n) {
        ...
        derivatives[n] =
            d1powers * accumulate(coef.crend() - diff_t(n - 1),
                                       coef.crend(),
                                       coef[n - 1],
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> lgamma(fvar<RealType, Order> const& cr) {
  using std::lgamma;
  using root_type = typename fvar<RealType, Order>::root_type;
  constexpr size_t order = fvar<RealType, Order>::order_sum;
  root_type const x = static_cast<root_type>(cr);
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> sinc(fvar<RealType, Order> const& cr) {
  if (cr != 0)
    return sin(cr) / cr;
  using root_type = typename fvar<RealType, Order>::root_type;
  constexpr size_t order = fvar<RealType, Order>::order_sum;
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> sinh(fvar<RealType, Order> const& cr) {
  BOOST_MATH_STD_USING
  using root_type = typename fvar<RealType, Order>::root_type;
  constexpr size_t order = fvar<RealType, Order>::order_sum;
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> tanh(fvar<RealType, Order> const& cr) {
...
template <typename RealType, size_t Order>
BOOST_MATH_CUDA_ENABLED fvar<RealType, Order> tgamma(fvar<RealType, Order> const& cr) {
  using std::tgamma;
  using root_type = typename fvar<RealType, Order>::root_type;
  constexpr size_t order = fvar<RealType, Order>::order_sum;
  if BOOST_AUTODIFF_IF_CONSTEXPR (order == 0)
...
BOOST_MATH_CUDA_ENABLED inline BOOST_MATH_CONSTEXPR destination_t real_cast(detail::autodiff_fvar_type<RealType, Order> const& from_v)
